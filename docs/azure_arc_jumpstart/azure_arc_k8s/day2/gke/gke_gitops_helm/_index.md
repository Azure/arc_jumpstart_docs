...
## Perform Helm-based GitOps flow on Google Kubernetes Engine (GKE) as an Arc Connected Cluster
...
- To keep your local environment clean and untouched, we will use [Google Cloud Shell](https://cloud.google.com/shell) to run the [*az_k8sconfiguration_helm_gke*](https://github.com/microsoft/azure_arc/blob/main/azure_arc_k8s_jumpstart/gke/gitops/helm/az_k8sconfiguration_helm_gke.sh) shell script against the GKE connected cluster.

- Edit the environment variables in the [*az_k8sconfiguration_helm_gke*](https://github.com/microsoft/azure_arc/blob/main/azure_arc_k8s_jumpstart/gke/gitops/helm/az_k8sconfiguration_helm_gke.sh) shell script to match your parameters, upload it to the Cloud Shell environment and run it using the _`. ./az_k8sconfiguration_helm_gke`_ command.
...
- Once the script will complete it's run, you will have two GitOps configuration created and all the resources deployed in your Kubernetes cluster.
    ...
    ![New GitOps configuration](./16.png)
...
- Before kicking off the GitOps flow, verify and "zoom in" to the Kubernetes resources deployed by running a few `kubectl` commands.
...
- Show the Flux operator pods.
    ...
    ![Get pods of namespace flux-system ](./19.png)
...
- Show 3 replicas and service of the "Hello Arc" application.
    ...
    ![Get pods and services of namespace hello-arc](./20.png)
...
- Show Nginx controller replicas and Kubernetes Service (Type _LoadBalancer_).
    ...
    ![Get pods and services of namespace ingress-nginx](./21.png)
...
- Show Nginx rule which will route the traffic to the "Hello Arc" application from outside the cluster.
    ...
    ![Get ingress of namespace hello-arc](./22.png)
...
- To show the above flow, open 2 (ideally 3) side-by-side windows:
  ...
  - Azure Cloud Shell running the command
    ...
    ![Get pods of namespace hello-arc -w](./23.png)
  ...
  - The external IP address of the Kubernetes Service is seen using the _`kubectl get svc -n ingress-nginx`_ command.
    ...
    ![Get services of namespace ingress-nginx](./24.png)
...
To delete the GitOps configuration and it's respective Kubernetes resources, edit the environment variables to match the Azure Arc Kubernetes cluster and Resources in the [az_k8sconfiguration_helm_cleanup](https://github.com/microsoft/azure_arc/blob/main/azure_arc_k8s_jumpstart/gke/gitops/helm/az_k8sconfiguration_helm_cleanup.sh) script, upload it to Cloud Shell and run it using the _`. ./az_k8sconfiguration_helm_cleanup.sh`_ command.
