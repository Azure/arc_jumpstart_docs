---
type: docs
title: "Apply GitOps configuration on GKE as an Arc Connected Cluster using Azure Policy"
linkTitle: "Apply GitOps configuration on GKE as an Arc Connected Cluster using Azure Policy"
weight: 4
description: >
---

## Apply GitOps configuration on GKE as an Arc Connected Cluster using Azure Policy

The following Jumpstart scenario will guide you on how to enable [Azure Policy for Kubernetes](https://learn.microsoft.com/azure/governance/policy/concepts/policy-for-kubernetes#:~:text=Azure%20Policy%20extends%20Gatekeeper%20v3,Kubernetes%20clusters%20from%20one%20place.) on a Google Kubernetes Engine (GKE) cluster that's projected as an Azure Arc connected cluster as well as how to create GitOps policy to apply on the cluster.
...
* To verify successful registration, run either the below Azure CLI or Azure PowerShell command.
    ...
    ![Installing the Azure Policy add-on resource provider](./04.png)

    ![Installing the Azure Policy add-on resource provider](./05.png)
...
* Before assigning the policy, in the Azure portal, click the *Configuration* setting in your GKE connected cluster. Notice how no GitOps configuration are deployed.

    ![No GitOps configuration](./12.png)

* In the Search bar, look for *Policy* and click on *Definitions* which will show you all the available Azure policies.
...
* Now that the policy is in compliant state, first verify the GitOps configuration. In the Azure portal click the GKE connected Azure Arc cluster and open the Configurations settings.

    ![Newly created GitOps configuration](./26.png)

    ![Newly created GitOps configuration](./27.png)

    ![Newly created GitOps configuration](./28.png)

* To verify the "Hello Arc" application and it's component has been deployed, In the Google Cloud Shell, run the below commands.
    ...
    ![Running commands to verifying successful deployment](./29.png)
...
* Delete the GKE cluster as described in the [deletion instructions](/azure_arc_jumpstart/azure_arc_k8s/gke/gke_terraform/).
